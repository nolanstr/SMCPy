...
class MultiSourceNormalWeighted(Normal):
    ...
    def __call__(self, inputs):
        ...
        log_likes = []
        start_idx = 0
        for i, (segment_len, segment_std_dev) in enumerate(
                                zip(*[self._args[0], std_devs])):
            ...
            output_segment = output[:, start_idx : start_idx + segment_len]
            data_segment = self._data[start_idx : start_idx + segment_len]
            log_likes.append(self.weights[i]*\
                            self._calc_normal_log_like(output_segment,
                                                        data_segment,
                                                        segment_std_dev ** 2))
