...
class ParallelParticles(Checks):
    '''
    ...
    def compute_covariance(self):
        ...
        '''
        cov = np.zeros((self._parallel, self.params.shape[2],
                                            self.params.shape[2]))
        ...
        #if cov.shape == ():
        #    cov = cov.reshape(1, 1)
        pos_def = np.array([self._is_positive_definite(cov[i]) for i in \
                                                    range(self._parallel)])
        if np.any(~pos_def):
            warnings.warn('Covariance matrix is not positive definite; setting '
                          'off-diagonal terms to zero.')
            for i in np.where(~pos_def)[0]:
                cov[i] = np.eye(cov[i].shape[0]) * np.diag(cov[i])
